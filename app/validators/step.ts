...
export const createStepValidator = vine.create({
  ...
  new_medias: vine.array(vine.file({ size: '16mb', extnames: ['jpg', 'jpeg', 'png', 'webp', 'heic', 'heif'] })).maxLength(15).optional(),
  new_medias_crop: vine.string().optional(),
...
export const updateStepValidator = vine.create({
  ...
  end_date: vine.date({ formats: ['YYYY-MM-DD'] }),
  new_medias: vine.array(vine.file({ size: '16mb', extnames: ['jpg', 'jpeg', 'png', 'webp', 'heic', 'heif'] })).maxLength(15).optional(),
  old_medias: vine.array(vine.string().regex(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(jpg|jpeg|png|webp|heic|heif)$/i)).maxLength(15).optional(),
  medias_order_refs: vine.string().optional(),
  new_medias_crop: vine.string().optional(),
  old_medias_crop: vine.string().optional(),
  link: vine.string().url().optional(),
  travel_id: vine.string().uuid(),
  travel_title: vine.string()
})
